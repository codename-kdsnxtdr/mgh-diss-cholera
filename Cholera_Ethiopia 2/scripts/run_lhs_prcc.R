...
source("scripts/cholera_dpm_vax_sa.R")
...
cholera_dpm_vax_safun <- function(parameters) {
  parameters$zeta <- 10^(parameters$zeta)
  parameters$K <- 10^(parameters$K)
  
  with(as.list(c(parameters)), {
    out <- tryCatch({ # produce out object containing transmission model solutions across all timesteps given defined parameters
...
parm.ranges = list(
  ...
  zeta = c(9,12), # log-base severe case human bacterial shedding rate
  K = c(2,10), # log-base of natural environmental carrying capacity 
...
f_name <- "outputs/lhs.prcc.sims_logOK.rds" # define a save file name
...
lhs.sims$zeta <- 10^(lhs.sims$zeta)
lhs.sims$K <- 10^(lhs.sims$K)
